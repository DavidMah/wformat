...
def setup(options = [])
  @wall_builder.should_receive(:prepare_image).once.with("42", anything, anything).and_return(@im_one)
  @wall_builder.should_receive(:prepare_image).once.with("39", anything, anything).and_return(@im_two)
  ...
  @wall_builder.should_receive(:place_onto).once.with(0, 0, @im_one, @target)    if options.include? 'place'
  @wall_builder.should_receive(:place_onto).once.with(1920, 0, @im_two, @target) if options.include? 'place'
...
describe WallMerger do
  ...
  describe "#merge" do
    ...
    it "should place two 1000x1000 images alongside each other" do
      ...
      @wall_builder.should_receive(:place_onto).once.with(1000, 0, @im_two, @target)
      @wall_builder.merge("42", "39", {'width_one' => 1000, 'width_two' =>1000, 'height_one' => 1000, 'height_two' => 1000})
    end

    it "should use the client requested filename" do
      setup(['images', 'size', 'place'])
      @target.should_receive(:write).once.with("garpley_title")
      @wall_builder.merge("42", "39", {'title' => "garpley_title"})
    end
    ...
    it "should rightward shift thin images" do
      ...
      @wall_builder.should_receive(:place_onto).once.with(200, 0,  @im_one, @target)
      @wall_builder.should_receive(:place_onto).once.with(2000, 0, @im_two, @target)
      @wall_builder.merge("42", "39")
    end
    ...
    it "should downward shift short images" do
      ...
      @wall_builder.should_receive(:place_onto).once.with(0, 100, @im_one, @target)
      @wall_builder.should_receive(:place_onto).once.with(1920, 200, @im_two, @target)
      @wall_builder.merge("42", "39")
    end
    ...
    it "should scale and shift overall small images" do
      ...
      @wall_builder.should_receive(:place_onto).once.with(480, 0, @im_one, @target)
      @wall_builder.should_receive(:place_onto).once.with(1920, 300, @im_two, @target)
      @wall_builder.merge("42", "39")
    end
  end
end
